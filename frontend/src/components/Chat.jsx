...
const MODE_LABELS = {
  consumer: { title: "OpenClaw Assistant", sub: "Plain language automation" },
  maker:    { title: "Workflow Builder",    sub: "Trigger → Condition → Action" },
  poweruser:{ title: "Power Console",       sub: "JSON workflow output" },
}

const SUGGESTIONS = {
  consumer: [
    "Turn on my bedroom light when motion is detected at night",
    "Alert me when the temperature goes above 30°C",
    "Water my plants every morning if the soil is dry",
  ],
  maker: [
    "Motion sensor triggers lights after 10PM with 5 min auto-off",
    "Temperature > 30 → fan on + send notification",
    "Door sensor opens → all lights on + fan on",
  ],
  poweruser: [
    "MQTT motion event → time condition 22:00–06:00 → light on → delay 300s → off",
    "Temp sensor > 30 → fan device_control max + notify user",
    "Security mode: motion + night → lights flash + camera + alert",
  ],
}
...
export default function Chat({ mode, messages, setMessages, onWorkflowSaved }) {
  ...
  const [savedWorkflowName, setSavedWorkflowName] = useState(null)
  ...
  async function send(text) {
    ...
    try {
      ...
      if (data.workflow) setPendingWorkflow(data.workflow)
    } catch {
      setMessages([...newMessages, {
        role: 'assistant',
        content: "Couldn't reach the backend. Make sure it's running on port 8000."
      }])
    } finally {
      setLoading(false)
    }
  ...
  async function saveWorkflow() {
    ...
    try {
      ...
      setSavedWorkflowName(pendingWorkflow.name)
      setPendingWorkflow(null)
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-4">
        ...
        {messages.map((m, i) =>
          m.role === 'user'
            ...
            : <BotMessage key={i} text={m.content} mode={mode} />
        )}
          ...
          <div className="animate-fadeup mx-auto max-w-md">
            <div className="px-5 py-4 rounded-2xl border border-claw-amber/30 bg-claw-amber/5">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <p className="text-claw-amber text-xs font-body uppercase tracking-widest mb-1">Workflow detected</p>
              ...
              <button onClick={saveWorkflow} disabled={savingWorkflow}
                ...
                {savingWorkflow ? 'Saving…' : '+ Save to workflows'}
              </button>
            </div>
          </div>
        )}
        ...
        {savedWorkflowName && (
          <div className="animate-fadeup mx-auto max-w-md px-4 py-3 rounded-xl
            ...
            ✓ "{savedWorkflowName}" saved to your workflows
          </div>
        )}
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full resize-none bg-claw-surface border border-claw-border
                ...
                focus:outline-none focus:border-claw-muted transition-colors duration-150 leading-relaxed"
            />
          </div>
